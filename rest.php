...
class Rest
{
	...
	public function validateRequest()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] !== 'GET')
		{
			...
			if (!isset($data['param']) || !is_array($data['param'])) {
				$this->throwError(API_PARAM_REQUIRED, "API param is required. e.g. \"param\"=array");
				$xparam = null;
			} else {
				$xparam  = $data['param'];
			}
			...
			if (!isset($_GET['page']) && !isset($_GET['limit'])) {
				// You may choose to throw an error or handle it differently for GET requests
				$this->throwError(API_PARAM_REQUIRED, "API param is required.");
				$xparam = null;
			} else {
				// For GET requests, param should be a comma-separated string or similar.
				$this->page = $_GET['page'];
	...
	public function validateToken()
	{
		try {
			...
			$stmt = $this->dbConn->prepare("SELECT * FROM [STORE_AUDIT_API_USERS] WHERE id = :userId");
	...
	public function throwError($code, $message)
	{
		...
		$errorMsg = json_encode(['response' => ['status' => $code, 'message' => $message]]);
		echo $errorMsg;
		exit;
	}
